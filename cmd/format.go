...
func fixObjectSize(s int) int {
	/*
		Determine and calculate object size
	*/
	var bits uint
	for s > 1 {
		bits++
...
func formatFlags() *cli.Command {
	/*
		Specify arguments to format a filesystem volume
	*/
	var defaultBucket = "/var/spinel"
	if runtime.GOOS == "darwin" {
		...
		defaultBucket = path.Join(homeDir, ".spinel", "local")
	}
	return &cli.Command{
		Name:      "format",
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				Name:  "storage",
				Value: "file",
				Usage: "Object storage type (e.g. s3)",
			},
			&cli.StringFlag{
				Name:  "bucket",
				Value: defaultBucket,
